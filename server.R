...
shinyServer(function(input, output) {
  ...
  read_table1 <- reactive({
    ...
    if (input$S3 == T){
        ...
        #inFile <- input$data
        file1 <- file <- s3read_using(FUN = read.csv, object = "s3://piyushxx/piyushxx/airquality.csv")  
        #file1 <- read.csv("https://piyushxx.s3.ap-south-1.amazonaws.com/piyushxx/airquality.csv")
        file2 <- file1
